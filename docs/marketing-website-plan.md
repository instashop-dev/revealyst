...
- **Custom domain — resolved.** `revealyst.com` is purchased and will be linked before launch. Remaining work (pre-launch checklist, ideally before or with PR 2): attach the custom domain to the Worker in Cloudflare, flip `SITE_URL` to `https://revealyst.com`, and 301-redirect `revealyst.thapi.workers.dev` → `revealyst.com` so canonical/OG/sitemap URLs and launch backlinks (directories, HN, PH) all accrue to the real domain.
