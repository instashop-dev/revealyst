# 0018 — GitHub Copilot connector, agentic metrics, and the AI-credits unit
...
- **`CANONICAL_METRICS` += four keys**, seeded by migration `0020`:
...
per connection. Registration flips every "Soon" surface to present-tense
automatically (the landing "Connects" strip derives from the registry).
...
- **`drizzle/0020_seed-agentic-and-credits-metrics.sql`** — seeds the 4 catalog
  rows (idempotent `ON CONFLICT DO NOTHING`). Migration numbering is independent
  of ADR numbering (0020 vs 0018).
...
- **Glossary/methodology (W3-content):** `METRIC_REFERENCE` gained the four
  entries; the mirror test now reads `0007` + `0020`.
