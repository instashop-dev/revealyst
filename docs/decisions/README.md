...
| 0054 | `remove-polled-connectors-agent-first` | Remove the polled admin-API connectors + `/connections` surface + renewal reminders; go agent-first (Settings → Devices). Reverses the connector halves of 0013/0022/0032. Freeze-in-place (no migration, no frozen path touched); registry emptied + poll dispatch deleted; connector modules kept as inert test fixtures; coverage narrows to Claude Code (accepted) | Accepted |
