...
| 0057 | `ai-tool-used-signal` | `ai_tool_used`: the content-free "which AI desktop apps are open" signal (mig 0045; discharges ADR 0055 for this key). One new `CANONICAL_METRICS` key + closed AI-app enum + new `tool` dimKind; app identity only (no window titles/command lines), fail-closed closed-enum check on device AND server; `directional` ceiling (not an OTel marker). Collector ships complete but DORMANT — live emission gated on D-DA-8 | Accepted |
| 0058 | `desktop-token-rotation` | Desktop-agent token rotation (T7.2; retires D-DA-4): device token becomes a refresh credential only, ordinary calls use a short-lived 15-min HMAC-SHA256 JWT minted at `POST /api/desktop/auth/refresh`; server both signs and verifies (no dep, new `DESKTOP_ACCESS_TOKEN_SIGNING_KEY` secret); `authenticateDesktopBearer` accepts BOTH creds during rollout; no migration, no new table (stateless, revocation via the paused-connection check + short TTL) | Accepted |
| 0059 | `worktype-classifier` | On-device work-type classifier (the D-DA-5 build; mig 0046): a new `worktype` family + `task_category`/`iteration_depth`/`verification_behavior` keys + closed task-category enum; deterministic borrow-and-drop Rust classifier reads prompt text ON-DEVICE and emits only labels/counts, fail-closed closed-enum check on both ends, `directional` ceiling. Honesty line changes "never reads" → "never LEAVES this computer". Wired-not-live (rule-2 fixture + pack activation still gated) | Accepted |
| 0060 | `connector-scoped-ingest` | Connector-scoped ingest: add `source_connector` to the frozen `metric_records` natural key (mig 0047, pure PK rebuild — no backfill). Family-scoped window-delete so one source's restatement no longer clobbers a sibling's overlapping days (fixes the D-DA-8 hazard); MAX-collapse read boundary keeps single-source orgs byte-identical; closed optional `source` client discriminator. Unblocks live Claude-export import → sync; #7/#9 live emission stays dormant | Accepted |
| 0061 | `team-goals` | Team goal / review period (`team_goals`, TMD P1a; mig 0048): org-scoped, optional `team_id` (NULL = org-wide), composite tenant FK mirroring `team_insights`; closed-enum `metric_slug` as text (not pg enum, capability target stays additive); nullable `baseline` (invariant b — never fabricate); one active goal per scope via archive-then-insert CAS + two partial unique indexes; not a self-view surface (holds only the manager's own `owner_user_id`); P1b setter/ordering/header deferred | Accepted |
| 0062 | `initiatives` | Initiatives + named participants (`initiatives` + `initiative_participants`, TMD P2a; mig 0049): the execution middle of the manager loop. CROSSES the self-view wall (named participants → manager) on the ADR-0038 mold, D-TCI-1 unblock — dual composite tenant FKs (participant → initiatives(org,id) AND people(org,id)); coded initiative library (§8 templates, no DB table); anti-gamification schema-shape + banned-phrasing guards (missions pattern). P2a = data layer only: count-only/uuid-only reads, NO named read yet (the manager-authorized named read + `MANAGER_AUTHORIZED_IDENTITY_SURFACES` registration + authz matrix ship in P2b). Purge: participants then initiatives, before people/teams | Accepted |
| 0064 | `capability-depth-spread` | Capability depth + spread (TMD P3 tail T3.3; mig 0051): two additive NULLABLE count-only columns on `team_capability_history` (`mastery_sum_bp` int + `mastery_sum_sq_bp` bigint) — the sufficient statistics (sum + sum-of-squares of mastery in basis points) from which pure `deriveDepthSpread` reconstructs team MEAN + population STDDEV. Stored for trend (founder "persist now") AND derived on-read for the live coverage card via the SAME `mastery.masteryStats()` + `deriveDepthSpread` (ADR 0046 drift-guard extended, parity-pinned). Exact integers (round4 mastery), nullable = honest "no depth data" not a fabricated 0, MIN_PEOPLE floor unchanged, no person id on the row. Dashboard batch +1 read (16→17, still depth 1). No new table → no new registration | Accepted |
| 0063 | `initiative-decision-log` | Initiative decision log (`initiative_decisions`, TMD P3 tail T3.2; mig 0050): the append-only who/why trail per initiative — records WHO decided and WHY, where `status_changed_at` recorded only WHEN. New table (keyed on initiative_id, not the person-keyed `manager_notes`). Closed `event` enum (launched/noted/completed/stopped; three auto-recorded on launch/complete/stop, `noted` is the manager-entered one); NO edit/delete (audit trail, not a mutable journal); `author_user_id` plain text no-FK (survives author departure, like initiatives.owner_user_id). Owner-OR-admin read+write (mirrors the review), impersonated writes 403, never feeds scoring (structural pin), anti-gamification schema-shape sweep. Fetched on demand in the review drawer (not in the count-only dashboard batch). Purge: before `initiatives`; FK-edge floor 25→26 | Accepted |
