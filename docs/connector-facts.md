...
- 2026-07-11: **OpenAI Auth erratum (ADR 0025):** live 403s prove the admin surface enforces per-endpoint scopes (`api.management.read` vs `api.usage.read`, gated separately) — the "No scopes — all-or-nothing" claim was wrong. NLV-O2 resolved (project-key rejection = 403, bare-string `error` body). Drove the `checkAdminKey` two-scope probe + transient-rethrow fix in the OpenAI connector.
