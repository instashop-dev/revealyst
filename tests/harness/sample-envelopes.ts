...
// reserves that directory for scrubbed real API responses). This is harness
// test input shaped exactly per docs/connector-facts.md §3's Claude Code
// Analytics field inventory, so the same E2E replays real recordings via
// tests/harness/vendor-payloads.ts the moment the founder commits them.
//
...
//  - two terminal types → feature_used distinct_dims > 1;
//  - estimated_cost as NUMBER cents (the documented money-format quirk);
...
export const sampleClaudeCodeEnvelope: RawPayloadEnvelope = {
  kind: "anthropic_console.claude_code",
  ...
  payload: {
    data: [
      ...
      // Duplicate (date, actor) row for api-key-1 (NLV-A8: dedup key not yet
      // verified live) — normalize must SUM these, not drop or double-count.
      {
        date: `${day1}T00:00:00Z`,
        actor: { type: "api_actor", api_key_name: "api-key-1" },
        organization_id: "org_scrub_1",
